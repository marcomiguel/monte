...
class Nota extends \Rpp\Pages\Page
{
    ...
    {
     if(empty(\Rpp\Services\Get\Content::part($nid,'_id'))) return  false;
     else return true;
    }
    ...
    public function set_position($position)
    {
       ...
       if($this->position > 1) 
       {
         $this->urlcanonical .= '/'.$this->position;
         $this->url_social .= '/'.$this->position;
       }
    }
    ...
    public function load_comscore() {
        ...
        $this->comscore = $this->slug_seccion[1] . '.' . $this->slug_seccion[2] . '.' . \Rpp\Services\Get\Content::node($this->nid)->tipo . '.' . $this->nid . '.interna';
