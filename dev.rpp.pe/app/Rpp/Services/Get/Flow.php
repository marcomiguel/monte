...
class Flow
{
  ...
  {
  	if(empty(static::$pattern_builder[$Pattern])) return static::$pattern_builder[$Pattern] = new SearchPatternBuilder( PatternFactory::$Pattern() );
  ...
  public static function home($limit=10,$init=0)
  {
    ...
    if( empty(static::$flow['home']) ) static::$flow['home'] = Cache::request()->get(Cache::get_conf()->cache_prefix->home);
  ...
  {
	if( empty(static::$flow['tag'][$slug]) ) static::$flow['tag'][$slug] = Cache::request()->get(Cache::get_conf()->cache_prefix->tag.$slug);
	else return array_slice(static::$flow['tag'][$slug],$init,$limit);

  if(Cache::request()->exists(Cache::get_conf()->cache_prefix->tag.$slug)){
  ...
  {
    if( empty(static::$flow['relatedtag'][$slug]) ) static::$flow['relatedtag'][$slug] = Cache::request()->get(Cache::get_conf()->cache_prefix->tag.'relatedtag.'.$slug);
    else return array_slice(static::$flow['relatedtag'][$slug],$init,$limit);

    if(Cache::request()->exists(Cache::get_conf()->cache_prefix->tag.'relatedtag.'.$slug)){
      ...
      return array_slice(static::$flow['relatedtag'][$slug],$init,$limit);
        ...
        static::$flow['related_tag_size'][$slug] = count(static::$flow['relatedtag'][$slug]);
        Cache::request()->save(
                         Cache::get_conf()->cache_prefix->tag.'relatedtag.'.$slug,
  ...
  {
    if( empty(static::$flow['seccion'][$slug]) ) static::$flow['seccion'][$slug] = Cache::request()->get(Cache::get_conf()->cache_prefix->seccion.$slug);
    else return array_slice(static::$flow['seccion'][$slug],$init,$limit);

    if(Cache::request()->exists(Cache::get_conf()->cache_prefix->seccion.$slug)) return  array_slice( static::$flow['seccion'][$slug] , $init , $limit ); 
    else{
  ...
  {
    if( empty(static::$flow['brandcontent']) ) static::$flow['brandcontent'] = Cache::request()->get(Cache::get_conf()->cache_prefix->brandcontent);
    else return  array_slice(static::$flow['brandcontent'],$init,$limit)  ; 

    if(Cache::request()->exists(Cache::get_conf()->cache_prefix->brandcontent)) return array_slice(static::$flow['brandcontent'],$init,$limit);
    else{
  ...
  public  static function autor($slug='general',$limit=50,$init=0)
  {
    if( empty(static::$flow['autor'][$slug]) ) static::$flow['autor'][$slug] = Cache::request()->get(Cache::get_conf()->cache_prefix->autor.$slug);
    else return  array_slice(static::$flow['autor'][$slug],$init,$limit)  ; 

    if(Cache::request()->exists(Cache::get_conf()->cache_prefix->autor.$slug)) return array_slice( static::$flow['autor'][$slug],$init,$limit) ;
    else {
      
     if($slug == 'general') static::$flow['autor'][$slug] = static::get_pattern('autor')->load();
     else static::$flow['autor'][$slug] = static::get_pattern('autor')->set_slug($slug)->load();

     if(empty(static::$flow['autor'][$slug])) static::$flow['autor'][$slug] = array();
     static::setting_cache('autor',$slug);
    }
    return array_slice(static::$flow['autor'][$slug],$init,$limit);
  } 
  ...
  public static function unset_seccion($slug,$nid)
  {
      static::seccion($slug);
      array_walk(static::$flow['seccion'][$slug], function ($k,$e) use ($nid,$slug) {
              if($k->_id==$nid) unset(static::$flow['seccion'][$slug][$e]);
            });
  }

  public static function unset_tag($slug,$nid)
  {
   $slug = str_replace('tag/','', $slug);
   static::tag($slug);
   array_walk(static::$flow['tag'][$slug], function ($k,$e) use ($nid,$slug) {
              if($k->_id==$nid)unset(static::$flow['tag'][$slug][$e]);
            });
  }


  public static function unset_autores($slug,$nid)
  {
    return true;
  }

  public static function unset_($slug,$nid)
  {
   return true;
  }
}
