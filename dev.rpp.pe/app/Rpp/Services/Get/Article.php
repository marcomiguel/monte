...
class Article
{
  ...
  {
  if( empty(static::$content[$nid]['node']) ) static::$content[$nid]['node'] = Cachejson::request()->get(Cachejson::get_conf()->cache_prefix->nota.$nid.'.node');
  else return static::$content[$nid]['node'];

  if(Cachejson::request()->exists(Cachejson::get_conf()->cache_prefix->nota.$nid.'.node'))return static::$content[$nid]['node'];
  else{
  	static::$content[$nid]['node'] = static::get_nota_repo($nid)->set_fields(explode(",", Cachejson::get_conf()->node->fields))->load();
  ...
  {
	  if( ! isset(static::$content[$nid][$part]) ) static::$content[$nid][$part] = Cachejson::request()->get(Cachejson::get_conf()->cache_prefix->nota.$nid.'.'.$part);
    else return static::$content[$nid][$part];

    if(Cachejson::request()->exists(Cachejson::get_conf()->cache_prefix->nota.$nid.'.'.$part))return static::$content[$nid][$part];
    else{
