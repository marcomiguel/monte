...
class Content 
{
  ...
  {
  if( empty(static::$content[$nid]['node']) ) static::$content[$nid]['node'] = Cache::request()->get(Cache::get_conf()->cache_prefix->nota.$nid.'.node');
  else return static::$content[$nid]['node'];

  if(Cache::request()->exists(Cache::get_conf()->cache_prefix->nota.$nid.'.node'))return static::$content[$nid]['node'];
  else{
  	static::$content[$nid]['node'] = static::get_nota_repo($nid)->set_fields(explode(",",Cache::get_conf()->node->fields))->load();
  ...
  {
	  if( ! isset(static::$content[$nid][$part]) ) static::$content[$nid][$part] = Cache::request()->get(Cache::get_conf()->cache_prefix->nota.$nid.'.'.$part);
    else return static::$content[$nid][$part];

    if(Cache::request()->exists(Cache::get_conf()->cache_prefix->nota.$nid.'.'.$part))return static::$content[$nid][$part];
    else{
  ...
  {
    Cache::request()->save(
                           Cache::get_conf()->cache_prefix->nota.$nid.'.'.$part,
                           static::$content[$nid][$part],
                           Cache::get_conf()->cache_time->nota
                          );
    
    Cachejson::request()->save(
                           Cachejson::get_conf()->cache_prefix->nota.$nid.'.'.$part,
                           static::$content[$nid][$part],
