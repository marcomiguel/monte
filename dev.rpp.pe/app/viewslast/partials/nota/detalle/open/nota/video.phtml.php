<?php $i=0;foreach (\Rpp\Services\Get\Content::part($this->model->nid,'contenido') as $contenido): ?> 
      ...
      <span> <?php echo @$contenido['credito']; ?> </span>
         ...
         <p><div id="swfplayer_<?php echo $contenido['youtube']['id']; ?>" class="nd_galeriaopen_<?php echo $contenido['youtube']['id']; ?>"></div>
         <div class="help-media">
      <?php echo @$contenido['video']['youtube']; ?> <?php if(!empty($contenido['via'])): ?>| fuente : {{contenido['via']}}<?php endif;?>
      <span> <?php echo @$contenido['credito']; ?> </span>
      </div></p>
