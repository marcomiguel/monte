...
t.html5imageupload = function(r, i) { //e, this
	this.element = i;
	this.options = t.extend(true, {}, t.html5imageupload.defaults, r, t(this.element).data(), t.html5imageupload.deg);
	...
	this.button.orientationLeft = '<button class="md-fab md-mini md-button md-primary md-default-theme" aria-label="Orientation Right"><img src="'+ CMSDATA.GLOBAL.URLSVG +'return10.svg" width="24" height:24px; /></button>';
	this.button.orientationRight = '<button class="md-fab md-mini md-button md-primary md-default-theme" aria-label="Orientation Left"><img src="'+ CMSDATA.GLOBAL.URLSVG +'returning7.svg" width="24" height:24px; /></button>';
...
t.html5imageupload.deg = { initOrientationLeft: 0};
t.html5imageupload.prototype = {
		...
		if (s.canvas == true) {
			...
			y.onload = function() {
				...
				b.onload = function() {
					...
					//console.log(b, 'b'); //IMG B64
					//console.log(l, 'l'); //LEFT
					//console.log(f, 'f'); //TOP
					//console.log(u, 'u'); //ANCHO
					//console.log(a, 'a'); //ALTO

					var angle = e.options.initOrientationLeft; //Angle
					g.save();
					if(angle === 90 || angle === -270){
						...
						g.translate(0, parseInt(a) + ((u-a)/2));
					}
	...
	imageOrientationLeft: function(deg90) {
		var e = this;
		var n = e.element;
		var r = e.image;
		var i = e.input;
		...
		this.options.initOrientationLeft = this.options.initOrientationLeft - deg90;
		if( this.options.initOrientationLeft === deg90*-4){
			this.options.initOrientationLeft = 0;
		}

		var objRotate = {
			//'-webkit-transform': 'rotate(' + this.options.initOrientationLeft + 'deg)',
			//'-moz-transform': 'rotate(' + this.options.initOrientationLeft + 'deg)',
			'transform': 'rotate(' + this.options.initOrientationLeft + 'deg)'
		};
		t(n).find(".ghost").css(objRotate);
		t(n).find(".main").css(objRotate);
		...
		/*console.log(r, 'RRRR');
		console.log(r, 'RRRR');
		console.log(r.data("useWidth"), 'useWidth');
		console.log(r.data("useHeight"), 'useHeight');

		t(r).css({width: r.data("useWidth"),height: r.data("useHeight"),top: r.data("left"),left: r.data("top")});
		e._ghost();
	...
	imageOrientationRight: function(deg90) {
		var e = this;
		var n = e.element;
		var r = e.image;
		var i = e.input;
	...
	imageReset: function() {
		...
		t(r).find(".ghost").css(objRotate);
		t(r).find(".main").css(objRotate);
		/*/Rotate*/
	...
	_tools: function() {
		...
		var s = n.options;
		//Orientation
		if (s.buttonOrientationLeft != false) {
			t(i).append(t(n.button.orientationLeft).on({"touchstart click": function(e) {
				//e.preventDefault();
				n.imageOrientationLeft(90);
			}}));
		}

		if (s.buttonOrientationRight != false) {
			t(i).append(t(n.button.orientationRight).on({"touchstart click": function(e) {
				e.preventDefault();
				n.imageOrientationRight(90);
			}}));
		}
